...
function createErrorStory(topic, error, retryCount) {
  return {
...
export async function generateStoryWithLLM(topic, userPreferences) {
...
`;
    ...
    while (retryCount < modelConfig.maxRetries && !storyJson) {
      try {
        const currentModel = retryCount >= modelConfig.maxRetries - 1 && modelConfig.fallbackModel ? modelConfig.fallbackModel : modelConfig.model;
