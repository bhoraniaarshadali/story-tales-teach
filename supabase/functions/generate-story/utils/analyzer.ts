...
import { getModelConfig, getModelForAttempt } from './modelConfig.ts';
...
export async function analyzeTopicEmotions(topic) {
  ...
  try {
    ...
    while (retries < modelConfig.maxRetries && !success) {
      try {
        const { model: currentModel, temperature: currentTemp } = getModelForAttempt(modelConfig, retries);
        console.log(`Analysis attempt ${retries + 1} for topic "${topic}" using ${currentModel}`);
        ...
        if (!response.ok) {
          console.warn(`API error on analysis attempt ${retries + 1}: ${response.status}`);
          retries++;
          if (retries < modelConfig.maxRetries) await new Promise(r => setTimeout(r, 1000));
          continue;
        }
        
        const data = await response.json();
        if (!data.choices || data.choices.length === 0) {
          console.warn(`API returned no choices on attempt ${retries + 1}`);
          retries++;
          continue;
        }
        ...
        if (jsonMatch) {
          ...
          console.warn(`No JSON found on attempt ${retries + 1}`);
          retries++;
        }
      } catch (requestError) {
    ...
    if (retries >= modelConfig.maxRetries) {
      console.warn(`All ${modelConfig.maxRetries} analysis attempts failed, using default`);
    }
