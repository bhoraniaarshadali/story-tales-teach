...
export const generateStory = async (topic: string): Promise<StoryResponse> => {
  try {
    ...
    // Validate the topic in the content
    if (!topicIsIncluded(data, topic)) {
      console.error('Generated story does not match the requested topic:', data);
      throw new Error(`Story doesn't appear to be about ${topic}`);
    }
...
// Check if the topic is actually included in the response content
function topicIsIncluded(story: any, topic: string): boolean {
  const topicLowerCase = topic.toLowerCase();
  
  // Check title, content, takeaway
  const contentHasTopic = 
